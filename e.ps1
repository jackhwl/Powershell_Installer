...
function CopyDirectorys
{
	if ($coreSiteExists)
	{
		...
		Copy-Item -path (Join-Path $InstallFolder web\Website) -destination (Join-Path $d.webSite.path ASP\Website) -recurse -force 
		Copy-Item -path (Join-Path $InstallFolder web\Data) -destination (Join-Path $d.webSite.path ASP\Data) -recurse -force 
	}
	else
	{
		echo "#####      Copy Website and Lib"
		Copy-Item -path (Join-Path $InstallFolder web) -destination (Join-Path $d.webSite.path ASP) -recurse -force 
...
foreach ($d in $c.configuration.sites.site)
{
	switch ($d.type)
	{
		"application" 
		{
	$appcmd = "$env:windir\system32\inetsrv\appcmd.exe set config """ + $d.webSite.siteName +"/"+$d.WebSite.applicationName+ """ ""/section:asp"" ""/enableParentPaths:false"""
	Invoke-Expression $appcmd
	exit
